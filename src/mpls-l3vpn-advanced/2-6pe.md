...
The BGP label tells the egress PE how to handle the IPv6 packet.
...
It is learned through LDP, RSVP-TE, or another IPv4-signaled MPLS mechanism.

In a basic CCIE EI-style lab, assume LDP.

```text
PE1 needs an LDP label to reach PE2's loopback.

Destination: 5.5.5.5/32
Label source: LDP
Purpose: transport across the MPLS core
```

The P routers only care about the transport label.

They do not care about the IPv6 prefix.

They do not care about the BGP label.

They do not care about the IPv6 header.

---

## BGP Label

The bottom label is advertised by MP-BGP.

It is bound to the IPv6 prefix.

```text
PE2 advertises:

IPv6 prefix: 2001:DB8:11:11::/64
BGP next hop: ::ffff:5.5.5.5
BGP label: 24
```

PE1 installs the route with two pieces of information:

```text
IPv6 destination: 2001:DB8:11:11::/64
Egress PE:        5.5.5.5
Inner label:      24
```

When PE1 forwards traffic to that IPv6 prefix, it pushes:

```text
Outer label: LDP label to 5.5.5.5
Inner label: BGP label advertised by PE2
```

---

## Why the Inner Label Is Needed

The inner label is not optional in normal 6PE operation.
...
If PE2 advertises implicit null for its IPv4 loopback, P2 pops the outer transport label.

The packet arriving at PE2 still needs to be labeled.

```text
Before PHP:

Outer label: LDP transport label
Inner label: BGP IPv6 label
Payload:     IPv6 packet
...
This is important because P2 does not need to send a native IPv6 packet.

P2 sends a labeled packet.

PE2 receives the inner label and knows the payload is IPv6.
...
```text
Destination 2001:DB8:11:11::11
Best route  2001:DB8:11:11::/64
Next hop    ::ffff:5.5.5.5
BGP label   24
```

PE1 resolves `5.5.5.5` in the IPv4 routing table.
...
CE1 LAN: 2001:DB8:10:10::/64
CE2 LAN: 2001:DB8:11:11::/64
...
```text
PE1(config)# ipv6 route 2001:DB8:10:10::/64 2001:DB8:12:12::10
```
...
PE1(config-router-af)# network 2001:DB8:1::/64
...
```text
PE2(config)# interface Ethernet0/0
...
PE2(config-router-af)# network 2001:DB8:11:11::/64
...
```text
CE1(config)# interface Loopback10
CE1(config-if)# ipv6 address 2001:DB8:10:10::10/64
```
...
```text
CE2(config)# interface Loopback11
...
PE1# traceroute 4.4.4.4 source l0 numeric
...
4.4.4.4         4        65000       8       8        1    0    0 00:03:44        0
```
...
>
> In most real networks, you'd use a dynamic routing protocol like OSPFv3 for PE-CE routing.
...
```
all-explicit-null:
 Advertise IPv6 Explicit Null for all prefixes.

explicit-null:
 Advertise IPv6 Explicit Null for PE2's local connected prefixes.
```

`2001:db8:2::/64` isn't directly connected to PE2, so I'll test with `all-explicit-null`:
...
PE2(config-router-af)# do sh bgp ipv6 unicast 2001:db8:2::/64
BGP routing table entry for 2001:DB8:2::/64, version 3
Paths: (1 available, best #1, table default)
  Advertised to update-groups:
     2         
...
```text
PE1 and PE2 exchange IPv6 routes,
but no labels are exchanged.
```
...
The neighbor is established.

```text
show bgp ipv6 unicast neighbors 5.5.5.5
```

The IPv6 MPLS label capability is missing.
...
Configure it on both PE routers.

### No IPv4 Route to the Remote PE Loopback

Problem:

```text
PE1 receives 2001:DB8:11:11::/64 with next hop ::ffff:5.5.5.5,
but PE1 cannot reach 5.5.5.5 in IPv4.
```
...
The route may appear, but it is not installed in the IPv6 routing table.
...
Fix the core IPv4 IGP.

### No MPLS Label to the Remote PE Loopback

Problem:

```text
PE1 can route to 5.5.5.5,
but does not have a label path to 5.5.5.5.
```

Symptoms:

```text
PE1# show ipv6 cef 2001:DB8:11:11::/64 detail
```

The route may not have the expected label stack.

Check:

```text
PE1# show mpls forwarding-table 5.5.5.5
PE1# show mpls ldp bindings 5.5.5.5 32
PE1# show mpls ldp neighbor
```

Fix LDP, MPLS interface configuration, or LDP-IGP synchronization issues.

### P Router Configured for IPv4 Routing but Not MPLS

Problem:

```text
The core has IPv4 reachability,
but a P router is not label switching.
```

Symptoms:

```text
PE1# traceroute 5.5.5.5 source 1.1.1.1
```

IPv4 traceroute works.

```text
PE1# show mpls forwarding-table 5.5.5.5
```

Label forwarding is missing or incomplete.
...
Also verify LDP neighbors.

### BGP Next Hop Changed by Route Reflector

Problem:

```text
RR changes the next hop to itself.
```

Symptoms:

```text
PE2 sees 2001:DB8:10:10::/64
Next hop: ::ffff:<RR-loopback>
```

If the RR is not in the forwarding path or is not a 6PE-capable forwarding node, traffic can fail.

Fix:

```text
Do not change next hop on reflected 6PE routes.
```

The next hop should remain the originating PE.

### Route Is in BGP but Not in the IPv6 RIB

Possible causes:

```text
BGP next hop is inaccessible.
No labeled path to the next hop.
Administrative distance conflict.
Route policy rejects the route.
The prefix is not considered valid.
```

Check:

```text
show bgp ipv6 unicast 2001:DB8:11:11::/64
show ipv6 route 2001:DB8:11:11::/64
show ip route 5.5.5.5
show ipv6 cef 2001:DB8:11:11::/64 detail
```
...
Check with large IPv6 pings.

```text
CE1# ping 2001:DB8:11:11::11 size 1500 source 2001:DB8:10:10::10
```

Fix:

```text
Increase MPLS MTU in the core.
Lower CE-facing IPv6 MTU.
Permit ICMPv6 Packet Too Big.
```

## 6PE Packet Walk

Use this packet walk for troubleshooting.
...
show bgp ipv6 unicast neighbors <neighbor>
show ipv6 route
show ipv6 cef <prefix> detail
show mpls forwarding-table
show mpls ldp neighbor
show mpls ldp bindings <remote-pe-loopback> 32
```
...
ping 2001:DB8:11:11::11 source 2001:DB8:10:10::10
```
...
traceroute 2001:DB8:11:11::11 source 2001:DB8:10:10::10
```
